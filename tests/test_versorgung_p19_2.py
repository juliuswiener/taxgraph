...
def test_kohorte_2005_max_tabelle():
    ...
    runner = _runner()
    assert _vfb(runner, versorgung_bemessungsgrundlage=12000,
...
def test_zuschlag_deckel_kleine_bg():
    ...
    runner = _runner()
    assert _vfb(runner, versorgung_bemessungsgrundlage=1500,
...
def test_zuschlag_deckel_sehr_kleine_bg():
    ...
    runner = _runner()
    assert _vfb(runner, versorgung_bemessungsgrundlage=200,
...
def test_vfb_ohne_hoechstbetrag_deckelung():
    ...
    runner = _runner()
    assert _vfb(runner, versorgung_bemessungsgrundlage=500,
...
def test_fehlende_bemessungsgrundlage():
    ...
    with pytest.raises(runner.VersorgungsfreibetragOffen):
        _vfb(runner, versorgung_bemessungsgrundlage=0,
             versorgungsbeginn_jahr=2025)
...
def test_fehlender_versorgungsbeginn_jahr():
    ...
    runner = _runner()
    with pytest.raises(runner.VersorgungsfreibetragOffen):
        _vfb(runner, versorgung_bemessungsgrundlage=12000,
...
def test_prozentsatz_nicht_ganzzahlig_exakt():
    ...
    runner = _runner()
    assert _vfb(runner, versorgung_bemessungsgrundlage=10000,
                versorgungsbeginn_jahr=2025) == 1287
...
def test_prozentsatz_1_dezimal_ab_2025():
    ...
    → 960+288 = 1248."""
    runner = _runner()
    assert _vfb(runner, versorgung_bemessungsgrundlage=36000,
