"""Ring-Level-§10 Abs.1 Nr.9 Schulgeld: der Sonderausgaben-Abzug mindert die festzusetzende
ESt (zahl_cent) am echten /ergebnis-Output. Differential-Test (mit Schulgeld vs Baseline) —
beweist die Ring-Verdrahtung + Erreichbarkeit (POST 201, nicht 400). Deckt Point B (gesamt Z.894)
+ Point C (rentner Z.1360). NULL LLM.

PER-KIND: Kz im Kind-Container, Ring liest per EM.instanzen. 2 Kinder UNGLEICHE Beträge
(10000+5000) → pro Kind: 2500+1500 = 4000€ Abzug (Einzelveranlagung, Deckel 2500/Kind).
Kz E0505607 (Sum), cent → ceiling.
"""
import json
import os
import sys
...
def _req(base: str, method: str, path: str, body: dict | None = None,
    ...
    """HTTP-Request mit optionalem Status-Check (1:1 aus test_p10_1_5_ring.py)."""
...
# ---- Kegel-Bausteine (Hochverdiener ~200k €, wie test_p10_1_5_ring.py) ----
...
#   Kind1 10000€ → 30%=3000, Deckel 2500 (Einzel) → 2500
#   Kind2  5000€ → 30%=1500, Deckel 2500 (Einzel) → 1500
#   Σ=4000€
SCHULGELD_2KINDER_EINZEL = [
    ("schulgeld", 1000000),          # Kind 1: 10.000 € → 30%=3000, Deckel 2500 → 2500
...
# die Splitting-Weiche im Accessor ist durch test_veranlagungsweiche_exakt abgedeckt.

# ===== TESTS =============================================================

def test_p10_1_9_schulgeld_ring_gesamt(base):
    """gesamt Hochverdiener, Einzelveranlagung: 2 Kinder ungleiche Beträge (10000+5000) → Σ=4000€ Abzug."""
    if not _catala_da():
...
def test_p10_1_9_schulgeld_ring_rentner(base):
    """rentner_gesamt Hochverdiener, Einzelveranlagung: 2 Kinder → Σ=4000€ Abzug."""
    if not _catala_da():
        pytest.skip("catala nicht verfügbar")
