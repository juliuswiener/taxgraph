...
def catala_p10_1a_realsplitting(s: dict) -> int:
    ...
    Accessor nimmt EUROS.

    Der Hoechstbetrag erhoeht sich um die KV/PV-Beitraege fuer den Ex-Ehegatten (S. 2), aber
    nur um die GEKUERZTEN: S. 2 verweist auf "Beitraege nach Absatz 1 Nummer 3", und
    § 10 Abs. 1 Nr. 3 Buchst. a S. 4 vermindert Krankenversicherungsbeitraege, aus denen ein
    Anspruch auf Krankengeld folgen kann, "um 4 Prozent". `kv_krankengeld` ist eine TEILMENGE
    von `kv_pv_beitraege`, kein zusaetzlicher Summand (die Pflegeversicherung begruendet keinen
    Krankengeldanspruch, ihr Anteil bleibt ungekuerzt).

    Seeds (pipeline/produktion/rules.yaml, alle sechs ueber clerk gegen das kompilierte Catala
    gefahren): 10k/0/0→10k, 15k/0/0→13805, 15k/2k/0→15k, 20k/0/0→13805,
    16k/2k/2k→15725 (ohne Kuerzung waeren es 15805), 20k/3k/2,5k→16705 (ohne: 16805).

    Die Kuerzung wird AUFGERUNDET. Der Accessor rechnet in ganzen Euro (api.py uebergibt
    Cent//100), Catala in Cent — bei krummen Betraegen weichen beide um bis zu einen Euro ab.
    Aufrunden der Kuerzung senkt den Deckel und damit den Abzug: die vorsichtige Richtung.
    Bei allen Seeds ist die Kuerzung glatt, dort stimmen beide exakt ueberein.

    HAND-FIX 2026-08-16 (Provenance in rules.yaml unter `hand_fix`): die Regel wurde von Hand
    korrigiert und deterministisch regatet, NICHT neu durch die Modell-Kaskade gefahren. Das
    judge_verdict im Snapshot stammt vom urspruenglichen Lauf und gilt der Fassung OHNE
    Kuerzung. Beleg sind die zwei Zitatanker plus syntax/typecheck/equivalence (A==B auf 6
    Rasterpunkten, davon 2 mit aktiver Kuerzung) und die 6 clerk-Seeds.
    """
    unterhaltsleistungen = int(s.get("unterhaltsleistungen", 0))
    kv_pv_beitraege = int(s.get("kv_pv_beitraege", 0))
    kv_krankengeld = int(s.get("kv_krankengeld", 0))
    kuerzung = -((-kv_krankengeld * 4) // 100)          # aufgerundet, s. Docstring
    deckel = 13805 + kv_pv_beitraege - kuerzung
