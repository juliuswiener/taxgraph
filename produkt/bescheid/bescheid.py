"""Rechenkern des Steuerbescheids — die Schicht, die api.py laut eigenem Vorspann nie sein wollte.

api.py sagt seit dem ersten Tag: "Keine Steuerlogik, keine zweite Wahrheit hier." Gemessen am
2026-08-18 standen dort 3579 Zeilen, davon 1874 in Funktionen mit `runner.catala_*`-Aufrufen.
Dieses Modul nimmt sie auf, damit aus dem Vorsatz eine Grenze wird, die man prüfen kann
(tests/test_bescheid_grenze.py).

RICHTUNG: bescheid kennt api NICHT. Der Kern ruft keine Endpunkt-Funktion zurück — gemessen,
nicht gehofft (test_kern_ruft_nicht_in_die_haut_zurueck). Deshalb ist die Extraktion mechanisch
und nicht ein Umbau: die Zeilen wandern unverändert, kein Logik-Edit.

EINGANG: die Funktionen bekommen alles, worauf sie rechnen, als Parameter (Feld-Snapshot,
Bindung, store, vz). Sie lesen keinen Fall von der Platte und schreiben keinen — das bleibt in
der Haut. `runner` wird wie bisher lazy innerhalb der Funktionen importiert; das ist kein
Stilfehler, sondern hält den Import von Catala aus dem Modul-Import heraus (mehrere Tests laufen
ohne verfügbaren Catala und würden sonst beim bloßen `import api` scheitern).

KEIN STAR-IMPORT: die Konstanten kommen einzeln aus api_constants. `from X import *` bindet
Werte für ALLE Namen auf einmal und unsichtbar — untersagt und geprüft in
tests/test_split_naht_gate.py, weil FAELLE per setattr gepatcht wird und eine Wert-Kopie die
Mutation nicht sähe. Hier wird FAELLE nicht gebraucht (der Kern liest keine Fälle), aber die
Regel gilt trotzdem: sie schützt vor der nächsten Konstante, die doch gepatcht wird.
"""
