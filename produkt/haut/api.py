...
# §35c energetische Sanierung (Abs.1 7%/6% + Energieberater 50%): 3 Felder, OPTIONAL (absent→0→
# over-tax-safe). Steuerermäßigung (mindert tarifliche ESt via steuerermaessigungen, Point B+C).
# WAR nicht in den Scheibe-felder registriert → api.py las die Felder, sie waren aber nie POSTbar
# (totes Wiring, Ring-Test-Fund) → jetzt in gesamt.felder UND RENTNER_FELDER (beide Ring-Punkte).
GESAMT_P35C = ("p35c_sanierungsaufwendungen", "p35c_ist_uebernaechstes_foerderjahr",
               "p35c_energieberater_aufwendungen")
# §10 Abs.1a Nr.1 Realsplitting (Unterhalt Ex-Ehegatte): 3 Felder, OPTIONAL (absent→0→over-tax-safe).
# Sonderausgabe (mindert zvE via g["sonderausgaben"], Point B Z.899 + Point C), Gate realsplitting_zustimmung.
# WAR — wie §35c — nicht in den Scheibe-felder registriert → api.py las die Felder, POST /event gab aber 400
# (totes Wiring, Ring-Test-Fund) → jetzt in gesamt.felder UND RENTNER_FELDER (beide Ring-Punkte).
...
RENTNER_FELDER = RENTNER_FELDER + GESAMT_FREIBETRAEGE + GESAMT_DBA + GESAMT_P23 + GESAMT_P33A + GESAMT_P32B + GESAMT_P35C + GESAMT_REALSPLITTING + P36_ANRECHNUNG + KIST_KONFESSION_FELDER + P22_NR3_EINKUENFTE
...
SCHEIBEN = {
    ...
    "gesamt": {
        "felder": (VV_GESAMT_FELDER + VV_ABS2_TATBESTAND + ("veranlagung", "bruttoarbeitslohn")
                   ...
                   + DHF_RING + DHF_BEDINGUNGEN + VERPFLEGUNG_TAGE + VERPFLEGUNG_GUARD
                   + UEBERNACHTUNG_RING + UEBERNACHTUNG_BEDINGUNGEN + ARBEITSMITTEL_RING
                   + P36_ANRECHNUNG + KIST_KONFESSION_FELDER),  # Weg ii: Abzüge + §24a/§24b + §21-Abs.2 + Vorsorge + §§13-18-Gewinn + §34c + §23 + §33a + §32b + §35c + §10 Abs.1a Realsplitting + §9-dHf/Verpflegung (B1) + §9-Übernachtung (A5) + §9-Arbeitsmittel-GWG (A6, gemischt §19) OPTIONAL
